...
export default defineComponent({
  ...
  setup(props, { attrs, slots, emit, expose }) {
    ...
    function getTableData(
      ...
      return fetchTableData({ ...queryParams, current, pageSize }, additionalParam)
        .then((response) => {
          ...
          if (before) {
            response[whereis] = before(response[whereis]);
          }
          dataSource.value.splice(0);
          response[whereis].forEach((v) => {
            dataSource.value.push(v);
          });
          if (after) {
            after(deepClone(dataSource.value));
          }
    ...
    reloadTableData();
    expose({ reloadTableData, dataSource: deepClone(tableMatirials.dataSource.value), getSelectedRowKeys, clearSelectedRowKeys });
